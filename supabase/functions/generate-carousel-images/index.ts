...
function buildAiPrompt(slide: any, isCover: boolean, stile?: string): string { const tema = String(slide?.theme || slide?.topic || "").trim(); const titolo = String(slide?.title || slide?.titolo || slide?.hook || "").trim(); const corpo = String(slide?.body || slide?.testo || slide?.sottotitolo || "").trim(); const kw = Array.isArray(slide?.keywords_stock) ? slide.keywords_stock.join(", ") : ""; if (stile === "neon") { return ["Immagine per social su fondo NERO ASSOLUTO.", tema ? ("Soggetto: un solo oggetto reale e riconoscibile che rappresenta in modo simbolico " + tema + ".") : "Soggetto: un solo oggetto reale legato alla cura del corpo.", titolo ? ("Il messaggio da evocare e: " + titolo + ".") : "", "Render 3D fotorealistico di quel singolo oggetto, completamente scontornato, sospeso al centro su fondo nero puro.", "Illuminazione al neon: alone turchese acceso attorno all oggetto; accenti rosso neon solo se il tema riguarda dolore, errore o problema.", "Superfici lucide con riflessi netti, altissimo dettaglio, resa premium da campagna pubblicitaria.", "VIETATO: qualunque testo, lettera, numero, logo, watermark, cornice, sfondo, pavimento, ombra a terra, persone o volti. Deve esserci solo l oggetto sul nero."].filter(Boolean).join(" "); } const parts = ["Fotografia editoriale professionale per un centro di fisioterapia e riabilitazione in Italia.", tema ? ("SOGGETTO CLINICO OBBLIGATORIO: la scena deve mostrare in modo riconoscibile il trattamento o la parte del corpo legata a: " + tema + ". Questo e il vincolo piu importante di tutti.") : "", titolo ? ("Contesto del messaggio: " + titolo + ".") : "", corpo ? ("Dettaglio: " + corpo.slice(0, 160) + ".") : "", kw ? ("Elementi utili: " + kw + ".") : "", "Stile: fotografia reale scattata con obiettivo 50mm, luce naturale morbida e diffusa, ambiente clinico moderno pulito e accogliente, palette calda e neutra (bianco, beige, legno chiaro, tocchi di verde salvia), profondita di campo cinematografica, altissimo dettaglio, aspetto autentico e non artefatto.", "Persone: adulti europei realistici, corporatura normale, espressione serena e credibile, abbigliamento sportivo neutro o divisa sanitaria semplice. Mani e volti anatomicamente corretti.", isCover ? "Inquadratura di grande impatto con soggetto centrale e ampio spazio negativo in alto e in basso per inserire del testo." : "Inquadratura naturale con spazio negativo laterale per inserire del testo.", "VIETATO nella immagine: qualunque testo, lettera, numero, scritta, logo, filigrana, watermark, interfaccia grafica, collage, bordi o cornici. Niente stile 3D, cartoon, illustrazione o rendering. Niente arti o dita deformate."]; return parts.filter(Boolean).join(" "); }
...
let aiModels: string[] = AI_MODEL_SETS["nano-2"]; let aiStyle = "foto"; if (brandId) { try { const { data: bm } = await supabaseAdmin.from("brands").select("image_model, slide_style").eq("id", brandId).maybeSingle(); const mk = String((bm as any)?.image_model || "nano-2"); if (AI_MODEL_SETS[mk]) aiModels = AI_MODEL_SETS[mk]; aiStyle = String((bm && bm.slide_style) || "foto"); console.log("Generatore immagini scelto: " + mk); } catch (e) { console.warn("image_model non letto:", e); } } const aiEnabled = (body as any).useAi !== false && aiModels.length > 0 && (Boolean(GEMINI_IMG_KEY) || Boolean(OPENAI_IMG_KEY));
...
console.log("AI decisione: enabled=" + aiEnabled + " | modelli=" + aiModels.join(",") + " | gemini=" + (GEMINI_IMG_KEY ? "presente" : "MANCANTE") + " | openai=" + (OPENAI_IMG_KEY ? "presente" : "assente") + " | brandId=" + String(brandId) + " | slides=" + slidesToProcess.length);
            const aiMap = new Map<number, string>(); if (aiEnabled) { const aiJobs = slidesToProcess.map(async (item: any) => { try { const gen = await generateAiImage(buildAiPrompt(item.slide, item.slide?.tipo === "cover", aiStyle), GEMINI_IMG_KEY, aiAspect, aiModels, OPENAI_IMG_KEY); if (!gen) { console.log("AI: nessuna immagine per slide " + item.originalIndex); return; } const savedAi = await saveAiBytes(gen.bytes, gen.contentType, supabaseAdmin, storagePath, item.originalIndex); if (savedAi) aiMap.set(item.originalIndex, savedAi); else console.log("AI: salvataggio su storage fallito per slide " + item.originalIndex); } catch (e) { console.log("AI ECCEZIONE slide " + item.originalIndex + ": " + String(e)); } }); await Promise.allSettled(aiJobs); console.log("Immagini AI generate: " + aiMap.size + " su " + slidesToProcess.length); console.log("AI DETTAGLIO TENTATIVI: " + AI_ERRORS.slice(-8).join("  ///  ")); AI_ERRORS.length = 0; }

async function generateAiImage(prompt: string, apiKey: string, aspectRatio: string, models: string[], openaiKey: string): Promise<{ bytes: Uint8Array; contentType: string } | null> { for (const model of models) { try { let res: Response; if (model.indexOf("gpt-image") === 0) { if (!openaiKey) { console.log("AI ERRORE: OPENAI_API_KEY mancante, salto " + model); continue; } res = await fetch(OPENAI_IMAGE_ENDPOINT, { method: "POST", headers: { "Authorization": "Bearer " + openaiKey, "Content-Type": "application/json" }, body: JSON.stringify({ model, prompt, size: "1024x1536", quality: "high", n: 1 }) }); } else { res = await fetch(AI_IMAGE_ENDPOINT, { method: "POST", headers: { "x-goog-api-key": apiKey, "Content-Type": "application/json" }, body: JSON.stringify({ model, input: [{ type: "text", text: prompt }], response_format: { type: "image", mime_type: "image/jpeg", aspect_ratio: aspectRatio, image_size: "2K" } }) }); } if (!res.ok) { const t2 = await res.text(); console.log("AI ERRORE " + model + " HTTP " + res.status + ": " + t2.slice(0, 400)); continue; } const json = await res.json(); const found = findImagePayload(json); const raw = found ? found.data : findAnyBase64(json); if (!raw) { console.log("AI ERRORE " + model + ": risposta senza immagine: " + JSON.stringify(json).slice(0, 400)); continue; } const bytes = b64ToBytes(raw); if (!bytes || bytes.length < 2000) { console.log("AI ERRORE " + model + ": payload troppo piccolo"); continue; } console.log("AI OK via " + model + " (" + bytes.length + " bytes)"); return { bytes, contentType: found && found.mime ? found.mime : "image/jpeg" }; } catch (e) { console.log("AI ERRORE " + model + " eccezione: " + String(e)); } } return null; }function findImagePayload(node: any, depth = 0): { data: string; mime: string } | null { if (!node || depth > 10) return null; if (Array.isArray(node)) { for (const c of node) { const r = findImagePayload(c, depth + 1); if (r) return r; } return null; } if (typeof node !== "object") return null; const n = node as any; const mime = String(n.mime_type || n.mimeType || ""); const data = n.data || n.bytesBase64Encoded || n.b64_json; if (typeof data === "string" && data.length > 500 && mime.indexOf("image/") === 0) return { data, mime }; for (const k of Object.keys(n)) { const r = findImagePayload(n[k], depth + 1); if (r) return r; } return null; }
...
const AI_MODEL_SETS: Record<string, string[]> = { "stock": [], "nano-2": ["gemini-3.1-flash-image", "gemini-3-pro-image"], "nano-pro": ["gemini-3-pro-image", "gemini-3.1-flash-image"], "gpt-2": ["gpt-image-2", "gemini-3.1-flash-image"] };
...
async function generateAiImage(prompt: string, apiKey: string, aspectRatio: string, models: string[], openaiKey: string): Promise<{ bytes: Uint8Array; contentType: string } | null> { for (const model of models) { try { let res: Response; if (model.indexOf("gpt-image") === 0) { if (!openaiKey) { console.warn("OPENAI_API_KEY non configurata, salto " + model); continue; } res = await fetch(OPENAI_IMAGE_ENDPOINT, { method: "POST", headers: { "Authorization": "Bearer " + openaiKey, "Content-Type": "application/json" }, body: JSON.stringify({ model, prompt, size: "1024x1536", quality: "high", n: 1 }) }); } else { res = await fetch(AI_IMAGE_ENDPOINT, { method: "POST", headers: { "x-goog-api-key": apiKey, "Content-Type": "application/json" }, body: JSON.stringify({ model, input: [{ type: "text", text: prompt }], response_format: { type: "image", mime_type: "image/jpeg", aspect_ratio: aspectRatio, image_size: "2K" } }) }); } if (!res.ok) { const t2 = await res.text(); console.warn("AI image " + model + " HTTP " + res.status + ": " + t2.slice(0, 300)); continue; } const json = await res.json(); const found = findImagePayload(json); const raw = found ? found.data : findAnyBase64(json); if (!raw) { console.warn("AI image " + model + ": nessuna immagine nella risposta"); continue; } const bytes = b64ToBytes(raw); if (!bytes || bytes.length < 2000) { console.warn("AI image " + model + ": payload troppo piccolo"); continue; } console.log("AI image OK via " + model + " (" + bytes.length + " bytes)"); return { bytes, contentType: found && found.mime ? found.mime : "image/jpeg" }; } catch (e) { console.warn("AI image " + model + " error:", e); } } return null; }

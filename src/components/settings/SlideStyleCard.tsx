...
export const SLIDE_STYLES = [
  { value: 'foto', label: 'Fotografico', hint: 'Sfondo chiaro con fotografie realistiche di studio. Adatto ai contenuti educativi.' },
  { value: 'neon', label: 'Notturno neon', hint: 'Fondo nero, titoli bianchi e turchesi, oggetto scontornato con bagliore. Per campagne e posizionamento.' },
  { value: 'referto', label: 'Referto', hint: 'Chiaro ed editoriale, banda colorata e numero grande. Sobrio e autorevole.' },
  { value: 'copertina', label: 'Copertina piena', hint: 'Immagine a tutta slide con titolo grande sovrapposto. Molto forte nel feed.' },
];
...
const SlideStyleCard: React.FC = () => {
  ...
  const save = async (next: string) => {
    ...
    toast({ title: 'Template impostato: ' + (scelto ? scelto.label : next) });
    reload();
  };
    ...
    <Card className="panel-card">
      ...
      <CardContent style={{ padding: '22px 24px' }}>
        <p className="text-xs mb-4" style={{ color: 'var(--ink3)' }}>
          Cambia la grafica delle slide e il tipo di immagine generata.
        </p>

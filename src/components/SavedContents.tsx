  ...
  if (isLoading) {
    return (
      <Card className="mt-6 sm:mt-8 backdrop-blur-enhanced">
        <CardHeader>
          <CardTitle className="text-foreground">Your Saved Contents</CardTitle>
        </CardHeader>
        <CardContent>
          ...
          {savedContents.map((content) => (
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-2 mb-3">
                ...
                {content.tone && (
                  <Badge variant="outline" className={`text-xs border ${getToneColor(content.tone)}`}>
                    {content.tone}
                  </Badge>
                )}
              </div>
        ...
        {savedContents.length < 3 && (
          <div className="mt-6 p-4 bg-gradient-to-r from-secondary/10 to-primary/10 rounded-lg border border-primary/20">
            <p className="text-primary text-sm text-center">
              💡 <strong>Pro Tip:</strong> Create at least 5-10 pieces of content to always have material ready to publish!
            </p>
          </div>
        )}
      </CardContent>
